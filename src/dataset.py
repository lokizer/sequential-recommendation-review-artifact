...
def left_pad_sequence(input_ids, max_len, strategy="zero", data_type="train"):
    """Pad on the left while keeping the most recent ``max_len`` items."""
    items = list(input_ids[-max_len:])
    ...
    # RepPad is a training augmentation; evaluation retains the standard input.
    effective_strategy = strategy
    if strategy == "reppad" and data_type != "train":
        effective_strategy = "zero"
    if effective_strategy in ("zero", "pre_fft_mask"):
        return [0] * pad_len + items
    ...
    mode = {
        ...
        "reflect": "reflect",
        "symmetric": "symmetric",
    }.get(effective_strategy)
    if mode is None:
        raise ValueError(f"Unsupported padding strategy: {strategy}")
    if len(items) == 1:
        mode = "edge"
    return np.pad(np.asarray(items), (pad_len, 0), mode=mode).astype(np.int64).tolist()
...
class RecDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_ids = left_pad_sequence(
            input_ids,
            self.max_len,
            strategy=getattr(self.args, "padding_strategy", "zero"),
