...
def parse_args():
    ...
    parser.add_argument(
        "--padding_strategy",
        default="zero",
        choices=("zero", "cyclic", "reflect", "symmetric", "reppad", "pre_fft_mask"),
        help="left-padding policy; reppad augments training only and pre_fft_mask keeps zero padding",
    )
    ...
    if args.model_type.lower() == 'bsarec':
        ...
        parser.add_argument("--freq_cutoff", default=5, type=int)
        parser.add_argument("--spectral_alpha", default=0.45, type=float)
        parser.add_argument("--high_pass_init", default=0.25, type=float)
        parser.add_argument("--wavelet_init", default=0.02, type=float)
        parser.add_argument(
            "--isolated_ablation_mode",
            default="without_local_wavelet_branch",
            choices=(
                "without_temporal_branch",
                "without_frequency_branch",
                "without_global_spectral_branch",
                "without_local_wavelet_branch",
                "without_adaptive_band_fusion",
                "without_time_frequency_gate",
            ),
        )
        parser.add_argument(
            "--strict_ablation_view",
            default="original",
            choices=("original", "validity_aware"),
        )
