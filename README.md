...
DuoSpectra studies how padded hidden states enter spectral sequence encoders.
It combines causal attention with global Fourier and local Haar processing,
using two asymmetric encoders:
...
provides the paper model, component-ablation implementation, processed data,
and entry points for training and evaluating the complete model. It is not a
release of every baseline or every experimental driver used in the paper.
Historical development variants, manuscript files,
...
module ablations. The two views differ in both boundary treatment and
temporal-frequency fusion; comparing them does not isolate masking alone.

## Artifact contents

| Resource | Location | Purpose |
| --- | --- | --- |
| Model and training code | `src/` | Asymmetric sequence encoders and optimization |
| Recorded configuration | `configs/paper.json` | Model identity, settings, and expected metrics |
| Training entry point | `scripts/train_duospectra.py` | Train one or both views |
| Evaluation entry point | `scripts/evaluate_duospectra.py` | Validation calibration and full-item testing |
| Integrity checks | `scripts/verify_release.py` | Encoder identity and processed-data hashes |
| Data documentation | `DATA.md` | Sources, record semantics, and preprocessing |

The study uses item-ID histories only. Review text, item descriptions, tag
labels, and pretrained language-model representations are not inputs. The
reported comparisons do not establish superiority over LLM-based recommenders.
...
These recorded values are reference targets, not results generated by the
integrity check. Reproducing them requires training and checkpoint evaluation.
